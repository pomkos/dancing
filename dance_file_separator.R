# This scrip takes all .csv files that start with "20", deletes first 3 rows, cleans the column names, and saves them in seven separate files in seven locations
...
get_files <- function(){
  setwd("C:/Users/albei/Nextcloud/Documents/PhD/Ridgel Lab/Dance Poster/Entropy Analysis/noraxon/Playground")
  folders <- list.dirs()
  ...
  for (x in folders){
    ...
    for (y in fold_list[[x]]){
      ...
      name_or[i] <- str_c("./Separate_dfs/Orientation", new, sep = "/")
      name_joint[i] <- str_c("./Separate_dfs/Joint", new, sep = "/")
      name_pitch[i] <- str_c("./Separate_dfs/Pitch", new, sep = "/")
      name_roll[i] <- str_c("./Separate_dfs/Roll", new, sep = "/")
      name_course[i] <- str_c("./Separate_dfs/Course", new, sep = "/")
      name_pelvis[i] <- str_c("./Separate_dfs/Pelvis", new, sep = "/")
      
      name_traj[i] <- str_c("./Separate_dfs/Trajectories", new, sep = "/")
    }
  }
