...
rt_hip_plt <- ggplot(df_pd, aes(
  x = Dance_Type,
  y = df_pd$`SamEn_Hip-RT-Abduction (deg)`,
  fill = Group )) + 
  ggtitle("Mean SamEn of RT Hip Abduction") +
  ksu_theme

lt_knee_plt <- ggplot(df_pd, aes(
  x = Dance_Type,
  y = df_pd$`SamEn_Knee-LT-Flexion (deg)`,
  fill = Group )) + 
...
pd_elbow_plt <- ggplot(df_pd, aes(x = Dance_Type)) + 
  geom_point(position = pos,
             aes(y = df_pd$`SamEn_Elbow-RT-Flexion (deg)`,
                 color = "6532")
  ) +
  geom_point(position = pos,
             aes(y = df_pd$`SamEn_Elbow-LT-Flexion (deg)`,
                 color = "8965"),
  ) +
  labs(title = "SamEn of PD Elbow Flexion",
       subtitle = "Left side most effected unless otherwise indicated",
       x = "Dance Types", 
       y = "SamEn") +
  theme_classic() +
